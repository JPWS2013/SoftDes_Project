...
class TextView(object):
	...
	def display_car(self):
		...
		for eachSensor in self.model.senseloc.keys():
			...
			if senseprop[0]=='Potentiometer':
				print "%s - %s degrees" % (senseprop[1], self.model.sensedict[eachSensor][-1])
	...
	def display_pot(self, sensor):
		print "Potentiometer has changed position to: %s degrees" % (self.model.sensedict[sensor.senseid][-1])

	def display_accel(self, sensor):
		print "Accelerometer has changed to: X: %s, Y: %s, Z: %s" % (self.model.sensedict[sensor.senseid][-1][0], self.model.sensedict[sensor.senseid][-1][1], self.model.sensedict[sensor.senseid][-1][2])